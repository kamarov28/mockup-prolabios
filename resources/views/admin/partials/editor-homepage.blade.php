  ...
  <div class="admin-card-header pb-0 border-bottom-0" style="background: var(--color-surface);">
    <ul class="nav nav-pills" id="homeTabs" role="tablist" style="gap: 8px;">
      <li class="nav-item" role="presentation">
        <button class="admin-btn admin-btn-outline active" id="hero-tab" data-bs-toggle="tab" data-bs-target="#hero-panel" type="button" role="tab" style="font-size: 0.72rem; padding: 8px 16px;"><i class="bi bi-image"></i> <span>Hero Banner</span></button>
      </li>
      <li class="nav-item" role="presentation">
        <button class="admin-btn admin-btn-outline" id="bento-tab" data-bs-toggle="tab" data-bs-target="#bento-panel" type="button" role="tab" style="font-size: 0.72rem; padding: 8px 16px;"><i class="bi bi-grid-1x2"></i> <span>Bento Grid Cards</span></button>
      </li>
      <li class="nav-item" role="presentation">
        <button class="admin-btn admin-btn-outline" id="sector-tab" data-bs-toggle="tab" data-bs-target="#sector-panel" type="button" role="tab" style="font-size: 0.72rem; padding: 8px 16px;"><i class="bi bi-diagram-3"></i> <span>Sector Finder</span></button>
      </li>
      <li class="nav-item" role="presentation">
        <button class="admin-btn admin-btn-outline" id="cta-tab" data-bs-toggle="tab" data-bs-target="#cta-panel" type="button" role="tab" style="font-size: 0.72rem; padding: 8px 16px;"><i class="bi bi-megaphone"></i> <span>Banner CTA Bawah</span></button>
      </li>
    </ul>
  </div>
    ...
    <div class="tab-content" id="homeTabsContent">
      ...
      <div class="tab-pane fade show active" id="hero-panel" role="tabpanel">
        ...
        <div class="row g-3 mb-4">
          <div class="col-md-4">
            ...
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="hero_badge" name="hero_badge" value="{{ old('hero_badge', $homeData['hero_badge'] ?? 'PRECISION LABORATORY SOLUTIONS') }}" required>
          </div>
          <div class="col-md-8">
            <label for="hero_title" class="admin-card-header-label mb-2">Hero Title / Slogan Utama</label>
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="hero_title" name="hero_title" value="{{ old('hero_title', $homeData['hero_title'] ?? '') }}" required>
        ...
        <div class="mb-4">
          ...
          <textarea class="form-control bg-dark text-white border-secondary border-opacity-20" id="hero_subtitle" name="hero_subtitle" rows="3" required>{{ old('hero_subtitle', $homeData['hero_subtitle'] ?? '') }}</textarea>
        </div>

        <div class="row g-3 mb-4">
          <div class="col-md-6">
            <label for="hero_cta_text" class="admin-card-header-label mb-2">Teks Tombol CTA Hero</label>
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="hero_cta_text" name="hero_cta_text" value="{{ old('hero_cta_text', $homeData['hero_cta_text'] ?? 'Explore Product Catalog') }}" required>
          </div>
          <div class="col-md-6">
            <label for="hero_cta_link" class="admin-card-header-label mb-2">Link URL Tombol CTA Hero</label>
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="hero_cta_link" name="hero_cta_link" value="{{ old('hero_cta_link', $homeData['hero_cta_link'] ?? '/produk') }}" required>
          </div>
        </div>
          ...
          @for($i = 0; $i < 4; $i++)
            ...
            <div class="col-md-6 col-lg-3">
              <div class="p-3 rounded-3 bg-black border border-secondary border-opacity-20 h-100">
                <span class="admin-badge admin-badge-accent mb-2">Slide #{{ $i + 1 }}</span>
                <div class="rounded border border-secondary border-opacity-20 overflow-hidden mb-3" style="aspect-ratio: 16/9;">
                ...
                <div>
                  ...
                  <input type="text" id="hero_image_url_{{ $i }}" class="form-control form-control-sm bg-dark text-white border-secondary border-opacity-20" name="hero_image_url_{{ $i }}" value="{{ $imgUrl }}">
                </div>
              </div>
            </div>
          @endfor
      ...
      <div class="tab-pane fade" id="bento-panel" role="tabpanel">
        ...
        <div class="row g-3 mb-4">
          <div class="col-md-6">
            ...
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="bento_title" name="bento_title" value="{{ old('bento_title', $homeData['bento_title'] ?? 'Infrastructure & Reliability Standards') }}" required>
          </div>
          <div class="col-md-6">
            <label for="bento_subtitle" class="admin-card-header-label mb-2">Subjudul Section Bento</label>
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="bento_subtitle" name="bento_subtitle" value="{{ old('bento_subtitle', $homeData['bento_subtitle'] ?? '') }}" required>
          </div>
        </div>
          ...
          @for($i = 0; $i < 4; $i++)
            ...
            <div class="col-md-6">
              <div class="p-3 rounded-3 bg-black border border-secondary border-opacity-20 h-100">
                ...
                <div class="mb-2">
                  <label for="bento_card_icon_{{ $i }}" class="form-label small text-secondary fw-bold mb-1">Icon Bootstrap</label>
                  <input type="text" id="bento_card_icon_{{ $i }}" class="form-control form-control-sm bg-dark text-white border-secondary border-opacity-20" name="bento_card_icon_{{ $i }}" value="{{ old('bento_card_icon_'.$i, $bCard['icon'] ?? 'bi-patch-check') }}" placeholder="bi-patch-check" required>
                </div>
                <div class="mb-2">
      ...
      <div class="tab-pane fade" id="sector-panel" role="tabpanel">
        ...
        <div class="row g-3 mb-4">
          <div class="col-md-6">
            ...
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="sector_title" name="sector_title" value="{{ old('sector_title', $homeData['sector_title'] ?? 'Interactive Sector Finder') }}" required>
          </div>
          <div class="col-md-6">
            <label for="sector_subtitle" class="admin-card-header-label mb-2">Subjudul Section Sector Finder</label>
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="sector_subtitle" name="sector_subtitle" value="{{ old('sector_subtitle', $homeData['sector_subtitle'] ?? '') }}" required>
          </div>
        </div>
      ...
      <div class="tab-pane fade" id="cta-panel" role="tabpanel">
        ...
        <div class="mb-3">
          ...
          <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="cta_banner_badge" name="cta_banner_badge" value="{{ old('cta_banner_badge', $homeData['cta_banner_badge'] ?? 'TECHNICAL PROCUREMENT SUPPORT') }}" required>
        </div>

        <div class="mb-3">
          <label for="cta_banner_title" class="admin-card-header-label mb-2">Judul Banner Konversi</label>
          <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="cta_banner_title" name="cta_banner_title" value="{{ old('cta_banner_title', $homeData['cta_banner_title'] ?? 'Require Custom Procurement or Project Quote?') }}" required>
        </div>

        <div class="mb-3">
          <label for="cta_banner_sub" class="admin-card-header-label mb-2">Subjudul Deskripsi Banner</label>
          <textarea class="form-control bg-dark text-white border-secondary border-opacity-20" id="cta_banner_sub" name="cta_banner_sub" rows="3" required>{{ old('cta_banner_sub', $homeData['cta_banner_sub'] ?? '') }}</textarea>
        </div>

        <div class="row g-3">
          <div class="col-md-6">
            <label for="cta_banner_btn_text" class="admin-card-header-label mb-2">Teks Tombol CTA</label>
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="cta_banner_btn_text" name="cta_banner_btn_text" value="{{ old('cta_banner_btn_text', $homeData['cta_banner_btn_text'] ?? 'Contact Sales / Request Quote') }}" required>
          </div>
          <div class="col-md-6">
            <label for="cta_banner_btn_url" class="admin-card-header-label mb-2">Link URL Tombol CTA</label>
            <input type="text" class="form-control bg-dark text-white border-secondary border-opacity-20" id="cta_banner_btn_url" name="cta_banner_btn_url" value="{{ old('cta_banner_btn_url', $homeData['cta_banner_btn_url'] ?? '/kontak') }}" required>
          </div>
        </div>
      </div>
