...
<head>
  ...
  <style>
    ...
    .badge-status { display: inline-block; padding: 4px 10px; background: #fffbe6; color: #d48806; border: 1px solid #ffe58f; border-radius: 4px; font-size: 12px; font-weight: 600; }
    ...
    .btn-track { display: inline-block; padding: 12px 28px; background: #FF4950; color: #ffffff !important; text-decoration: none; border-radius: 6px; font-weight: 700; font-size: 14px; text-align: center; margin-top: 10px; }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p>Official B2B Quotation System</p>
    </div>
    
    <div class="body">
      <div class="greeting">Halo {{ $rfq->pic_name }},</div>
      <div class="intro">
        Terima kasih telah mengajukan penawaran harga (RFQ) di PT. Prolabios Mitra Analitika. Pengajuan dari <strong>{{ $rfq->company_name }}</strong> telah berhasil kami terima dan sedang dievaluasi oleh Tim Sales & Procurement kami.
      </div>

      <div class="card-info">
        <table>
          <tr>
            <td class="label">Nomor RFQ:</td>
            <td class="val" style="color: #FF4950; font-size: 15px;">{{ $rfq->rfq_number }}</td>
          </tr>
          <tr>
            <td class="label">Perusahaan / Pabrik:</td>
          ...
          <tr>
            <td class="label">Status Penawaran:</td>
            <td class="val"><span class="badge-status">Menunggu Penawaran Resmi</span></td>
          </tr>
        </table>
      </div>

      <div style="font-weight: 600; font-size: 14px; margin-bottom: 10px; color: #1e293b;">Rincian Item Produk yang Diajukan:</div>
      ...
      <div style="text-align: center; margin: 30px 0 10px 0;">
        <a href="{{ route('rfq.track', ['number' => $rfq->rfq_number]) }}" class="btn-track">
          PANTAU STATUS PENAWARAN ANDA
        </a>
      </div>
    </div>
