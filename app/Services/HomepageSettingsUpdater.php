...
class HomepageSettingsUpdater
{
    ...
    /** @var list<string> */
    public const ALLOWED_SECTIONS = ['homepage', 'banners', 'contacts', 'general'];

    public function validate(Request $request, string $section): void
    {
        match ($section) {
            'homepage' => $this->validateHomepage($request),
            'banners' => $this->validateBanners($request),
    ...
    public function buildPatch(Request $request, string $section, array $homeData): array
    {
        return match ($section) {
            ...
            'banners' => ['patch' => $this->patchBanners($request, $homeData), 'error' => null],
    ...
    private function validateBanners(Request $request): void
    {
        $pages = ['products', 'sectors', 'services', 'info', 'contact'];
        $bannerRules = [];
        foreach ($pages as $p) {
            $bannerRules["{$p}_title"] = self::SHORT_TEXT_RULE;
            $bannerRules["{$p}_subtitle"] = self::TEXT_RULE;
     ...
     * @return array<string, mixed>
     */
    private function patchBanners(Request $request, array $homeData): array
    {
        $patch = [];
        $pages = ['products', 'sectors', 'services', 'info', 'contact'];
        foreach ($pages as $p) {
